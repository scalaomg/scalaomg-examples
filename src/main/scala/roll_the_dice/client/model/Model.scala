package roll_the_dice.client.model

import scalaomg.client.core.Client
import scalaomg.client.room.JoinedRoom
import roll_the_dice.client.{LeftMatchmaking, LeftRoom, PubSubMessage, PubSubNextTurn, PubSubRoomState, PubSubSetupGame, PubSubWin, Publisher}
import roll_the_dice.common.{ClientInfo, MatchState, ServerInfo, Team, Turn}
import roll_the_dice.common.MessageDictionary.{NextTurn, Roll, StartGame, Win}
...
class ModelImpl(serverInfo: ServerInfo) extends Model with Publisher {
  ...
  override def joinGameWithMatchmaking(desiredTeam: Team): Unit =
    client.matchmaker joinMatchmaking (roomName, ClientInfo(desiredTeam)) onComplete {
      ...
      case Failure(_) => println("Matchmaker failed to create a proper match")
    }
